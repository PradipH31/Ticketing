...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ErrorResponseDTO> handleNonexistentTicketIDException(RuntimeException exception) {
    return ResponseEntity.status(404).body(new ErrorResponseDTO(exception.getMessage(), LocalDateTime.now(),
  ...
  public ResponseEntity<ErrorResponseDTO> handleInvalidDataException(RuntimeException exception) {
    return ResponseEntity.status(400).body(new ErrorResponseDTO(exception.getMessage(), LocalDateTime.now(),
